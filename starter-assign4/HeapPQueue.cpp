...
//      error("out of range, you can not do the operation");
        allocatedSize *= 2;
        DataPoint* temp = temp = new DataPoint[allocatedSize];
        for(int i = 1; i <= logicalSize; i++){
            temp[i] = elems[i];
        }
        delete[] elems;   //deallocate array
        elems = temp;   //update pointer
    }
    if(logicalSize != 0){    //If no element enqueued
        int index = logicalSize + 1;   //ignore the elems[0]
        int parentIndex = index / 2;
        if(data.weight < elems[parentIndex].weight){
            do{
                elems[index] = elems[parentIndex];  //parent -> data
                elems[parentIndex] = data;  //data -> parent
                index = parentIndex;       //update
                parentIndex = index / 2;
            } while(data.weight < elems[parentIndex].weight && parentIndex != 0);
        } else {
            elems[index] = data;
        }
    } else {
...
DataPoint HeapPQueue::dequeue() {
    ...
    // memory management
//        int halfallocatedSize = allocatedSize / 2;
//        if(logicalSize + 1 <= halfallocatedSize){
//            DataPoint* newElems = new DataPoint[halfallocatedSize];
//            for(int i = 1; i <= logicalSize; i++){
//                newElems[i] = elems[i];
//            }
//            delete[] elems;
...
//        }
    ...
    // bubble down
        ...
        int Lindex = 2 * index;
        int Rindex = Lindex +1;
        while(Rindex <= logicalSize || Lindex <= logicalSize){
            if(Lindex == logicalSize){
                if(elems[Lindex].weight < elems[index].weight) {  //If right child is not exist
                    DataPoint temp = elems[Lindex];
                    elems[Lindex] = elems[index];
                    elems[index] = temp;
                    index = Lindex;
                }
                break;
            } else {   //If right child exist
                if(elems[Rindex].weight < elems[Lindex].weight){  //If right child is smaller than target
                    if(elems[Rindex].weight < elems[index].weight) {  //And is smaller than left
                        DataPoint temp = elems[Rindex];
                        elems[Rindex] = elems[index];
                        elems[index] = temp;
                        index = Rindex;
                    }
                } else if(elems[Lindex].weight < elems[index].weight) {
                    DataPoint temp = elems[Lindex];
                    elems[Lindex] = elems[index];
                    elems[index] = temp;
                    index = Lindex;
            ...
            if(index != Lindex && index != Rindex){  //If no smaller childs
                break;
            }
            Lindex = 2 * index;
            Rindex = Lindex +1;

...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    // error2: seats number < states number
    int numStates = populations.size();
    if(numStates > numSeats){
        error("no enough seats");
    }

    // creat a HeapPQueue  // Initially apportion
    HeapPQueue Popu;
    Map<string, int> seatApportion;
    for(const string& state : populations){
        Popu.enqueue({state, -((double)populations[state]) / sqrt(2)});
        seatApportion.put(state,1);
        numSeats--;
    }

    // Special case:
    if(numStates == numSeats){
        return seatApportion;
    }
    //
    while(numSeats--){
        DataPoint peek;
        peek = Popu.dequeue();
        int assigned = seatApportion[peek.name];
        assigned++;
        seatApportion[peek.name] = assigned;
        double New = -populations[peek.name] / sqrt(assigned*(assigned+1));
        peek.weight = New;
        Popu.enqueue(peek);
    }
//    while(numSeats--){
//        DataPoint peek;
//        peek = Popu.dequeue();
//        seatApportion[peek.name]++;
//        double New = compute(populations[peek.name],seatApportion[peek.name]);
//        peek.weight = -New;
//        Popu.enqueue(peek);
//    }
    return seatApportion;
